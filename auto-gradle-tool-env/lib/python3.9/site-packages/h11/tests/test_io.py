...
SIMPLE_CASES = [
    ...
    (
        (SERVER, SEND_RESPONSE),
        InformationalResponse(status_code=101, headers=[("Upgrade", "websocket")], reason=b"Upgrade"),
...
def test_writers_unusual() -> None:
    ...
    with pytest.raises(LocalProtocolError):
        tw(
            write_any_response,
            Response(status_code=200, headers=[("Connection", "close")], http_version="1.0"),
            None,
        )
...
def test_readers_unusual() -> None:
    ...
    tr(
        READERS[SERVER, SEND_RESPONSE],
        b"HTTP/1.0 200 OK\r\n" b"Foo:\r\n\r\n",
        Response(status_code=200, headers=[("Foo", "")], http_version="1.0", reason=b"OK"),
    )
    ...
    tr(
        READERS[SERVER, SEND_RESPONSE],
        b"HTTP/1.0 200\r\n" b"Foo: bar\r\n\r\n",
        Response(status_code=200, headers=[("Foo", "bar")], http_version="1.0", reason=b""),
    )
