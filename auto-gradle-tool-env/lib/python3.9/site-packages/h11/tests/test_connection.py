...
def test__keep_alive() -> None:
    ...
    assert not _keep_alive(Request(method="GET", target="/", headers=[], http_version="1.0"))  # type: ignore[arg-type]
...
def test_automatic_transfer_encoding_in_response() -> None:
    ...
    for user_headers in [
        ...
        p.send(
            ...
            expect=Response(status_code=200, headers=[("Transfer-Encoding", "chunked")]),
        )
...
def test_max_incomplete_event_size_countermeasure() -> None:
    ...
    assert get_all_events(c) == [
        Request(method="GET", target="/", http_version="1.0", headers=[("big", "a" * 4000)]),
        EndOfMessage(),
    ]
...
def test_close_different_states() -> None:
    ...
    p.send(
        CLIENT,
        Request(method="GET", target="/", headers=[("Host", "a"), ("Content-Length", "10")]),
    )
    with pytest.raises(LocalProtocolError):
