...
class CorsIssueDetails:
    ...
    def from_json(cls, json):
        return cls(
            ...
            resource_ip_address_space=network.IPAddressSpace.from_json(json["resourceIPAddressSpace"])
            if "resourceIPAddressSpace" in json
            else None,
            client_security_state=network.ClientSecurityState.from_json(json["clientSecurityState"])
...
class DeprecationIssueType(enum.Enum):
    ...
    CUSTOM_CURSOR_INTERSECTS_VIEWPORT = "CustomCursorIntersectsViewport"
    ...
    MEDIA_ELEMENT_AUDIO_SOURCE_NODE = "MediaElementAudioSourceNode"
    ...
    PAYMENT_REQUEST_SHOW_WITHOUT_GESTURE = "PaymentRequestShowWithoutGesture"
    ...
    RTP_DATA_CHANNEL = "RTPDataChannel"
...
class FederatedAuthRequestIssueReason(enum.Enum):
    ...
    CLIENT_METADATA_MISSING_PRIVACY_POLICY_URL = "ClientMetadataMissingPrivacyPolicyUrl"
...
class InspectorIssueDetails:
    ...
    def from_json(cls, json):
        return cls(
            cookie_issue_details=CookieIssueDetails.from_json(json["cookieIssueDetails"])
            if "cookieIssueDetails" in json
            else None,
            mixed_content_issue_details=MixedContentIssueDetails.from_json(json["mixedContentIssueDetails"])
            ...
            twa_quality_enforcement_details=TrustedWebActivityIssueDetails.from_json(
                json["twaQualityEnforcementDetails"]
            )
            if "twaQualityEnforcementDetails" in json
            else None,
            low_text_contrast_issue_details=LowTextContrastIssueDetails.from_json(json["lowTextContrastIssueDetails"])
            ...
            quirks_mode_issue_details=QuirksModeIssueDetails.from_json(json["quirksModeIssueDetails"])
            ...
            generic_issue_details=GenericIssueDetails.from_json(json["genericIssueDetails"])
            if "genericIssueDetails" in json
            else None,
            deprecation_issue_details=DeprecationIssueDetails.from_json(json["deprecationIssueDetails"])
            if "deprecationIssueDetails" in json
            else None,
            client_hint_issue_details=ClientHintIssueDetails.from_json(json["clientHintIssueDetails"])
