...
async def test_async_generator():
    ...
    assert await collect(tripler.async_multiplied(async_range(5))) == [0, 3, 6, 9, 12]
    ...
    for stop_after_turn in (1, 2, 3):
        ...
        if stop_after_turn == 2:
            ...
            with pytest.raises(RuntimeError) as info:
                gen.__del__()
            assert "awaited during finalization; install a finalization hook" in str(info.value)
        else:
...
async def test_gc_hooks_behavior(local_asyncgen_hooks):
    ...
    for _ in range(4):
        gc.collect()
    assert refA() is None
    assert events == ["yield 2 A", "after yield 2 A", "mock_sleep A", "yield 3 A", "unwind 3 A"]
    ...
    assert events == ["before aclose B", "mock_sleep B", "before aclose C", "unwind 3 C", "after aclose both"]
