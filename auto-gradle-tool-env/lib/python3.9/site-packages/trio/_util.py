...
def coroutine_or_error(async_fn, *args):
    ...
    try:
        ...
        if isinstance(async_fn, collections.abc.Coroutine):
            ...
            raise TypeError(
                ...
                "  nursery.start_soon({async_fn.__name__}, ...)  # correct!".format(async_fn=async_fn)
            ) from None
    ...
    if not isinstance(coro, collections.abc.Coroutine):
        ...
        if isasyncgen(coro):
            raise TypeError("start_soon expected an async function but got an async " "generator {!r}".format(coro))
