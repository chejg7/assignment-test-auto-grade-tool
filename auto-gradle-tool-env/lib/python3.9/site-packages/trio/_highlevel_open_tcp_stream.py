...
async def open_tcp_stream(host, port, *, happy_eyeballs_delay=DEFAULT_DELAY, local_address=None):
    ...
    if not targets:
        msg = "no results found for hostname lookup: {}".format(format_host_port(host, port))
        raise OSError(msg)
        ...
        try:
            ...
            if local_address is not None:
                ...
                try:
                    ...
                    raise OSError(
                        f"local_address={local_address!r} is incompatible " f"with remote address {sockaddr}"
                    )
    ...
    with close_all() as open_sockets:
        ...
        if winning_socket is None:
            ...
            msg = "all attempts to connect to {} failed".format(format_host_port(host, port))
