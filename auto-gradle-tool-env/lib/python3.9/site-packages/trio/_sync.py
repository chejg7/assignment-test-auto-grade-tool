...
class CapacityLimiter(metaclass=Final):
    ...
    def release_on_behalf_of(self, borrower):
        ...
        if borrower not in self._borrowers:
            raise RuntimeError("this borrower isn't holding any of this CapacityLimiter's tokens")
...
class Semaphore(metaclass=Final):
    ...
    def __repr__(self):
        ...
        return "<trio.Semaphore({}{}) at {:#x}>".format(self._value, max_value_str, id(self))
...
class _LockImpl:
    ...
    def __repr__(self):
        if self.locked():
            ...
            s1 = "unlocked"
            s2 = ""
        return "<{} {} object at {:#x}{}>".format(s1, self.__class__.__name__, id(self), s2)
...
class Condition(metaclass=Final):
    ...
    def statistics(self):
        ...
        * ``lock_statistics``: The result of calling the underlying
        ...
        """
        return _ConditionStatistics(tasks_waiting=len(self._lot), lock_statistics=self._lock.statistics())
