...
def _run_fn_as_system_task(cb, fn, *args, context, trio_token=None):
    ...
    if not trio_token:
        try:
            ...
            raise RuntimeError("this thread wasn't created by Trio, pass kwarg trio_token=...")
...
def from_thread_run(afn, *args, trio_token=None):
    ...
    **Locating a Trio Token**: There are two ways to specify which
    ...
    def callback(q, afn, args):
        ...
        context = contextvars.copy_context()
        try:
            trio.lowlevel.spawn_system_task(await_in_trio_thread_task, name=afn, context=context)
        except RuntimeError:  # system nursery is closed
            q.put_nowait(outcome.Error(trio.RunFinishedError("system nursery is closed")))
