...
class Process(AsyncResource, metaclass=NoPublicConstructor):
    ...
    @deprecated("0.20.0", issue=1104, instead="run_process or nursery.start(run_process, ...)")
...
async def open_process(command, *, stdin=None, stdout=None, stderr=None, **options) -> Process:
    ...
    if os.name == "posix":
        if isinstance(command, str) and not options.get("shell"):
            raise TypeError("command must be a sequence (not a string) if shell=False " "on UNIX systems")
        if not isinstance(command, str) and options.get("shell"):
            raise TypeError("command must be a string (not a sequence) if shell=True " "on UNIX systems")
...
async def _posix_deliver_cancel(p):
    try:
        ...
        warnings.warn(RuntimeWarning(f"tried to kill process {p!r}, but failed with: {exc!r}"))
