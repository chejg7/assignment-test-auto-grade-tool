    ...
    def _close(self, exc):
        if self._cancel_status is None:
            new_exc = RuntimeError(
                "Cancel scope stack corrupted: attempted to exit {!r} " "which had already been exited".format(self)
            )
        ...
        if scope_task._cancel_status is not self._cancel_status:
            ...
            if self._cancel_status.abandoned_by_misnesting:
                ...
                new_exc = RuntimeError(
                    ...
                    "from unrelated {!r}\n{}".format(self, scope_task, MISNESTING_ADVICE)
                )
                new_exc.__context__ = exc
                return new_exc
            else:
...
class Runner:
    ...
    def spawn_impl(self, async_fn, args, nursery, name, *, system_task=False, context=None):
...
def run(
    ...
    *args,
    ...
    runner = setup_runner(clock, instruments, restrict_keyboard_interrupt_to_checkpoints)
...
def unrolled_run(runner, async_fn, args, host_uses_signal_set_wakeup_fd=False):
    ...
    try:
        ...
        warnings.warn(
            RuntimeWarning("Trio guest run got abandoned without properly finishing... " "weird stuff might happen")
        )
...
async def checkpoint():
    ...
    task = current_task()
    task._cancel_points += 1
    if task._cancel_status.effectively_cancelled or (task is task._runner.main_task and task._runner.ki_pending):
