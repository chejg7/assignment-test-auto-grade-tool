...
async def test_parking_lot_basic():
    ...
    check_sequence_matches(record, [{"sleep 0", "sleep 1", "sleep 2"}, {"wake 0", "wake 1", "wake 2"}])
    ...
    async with _core.open_nursery() as nursery:
        ...
        lot.unpark(count=2)
        await wait_all_tasks_blocked()
        check_sequence_matches(record, ["sleep 0", "sleep 1", "sleep 2", {"wake 0", "wake 1"}])
        lot.unpark_all()
...
async def test_parking_lot_cancel():
    ...
    check_sequence_matches(record, ["sleep 1", "sleep 2", "sleep 3", "cancelled 2", {"wake 1", "wake 3"}])
