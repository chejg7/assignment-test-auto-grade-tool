...
async def getaddrinfo(host, port, family=0, type=0, proto=0, flags=0):
    ...
    async with _try_sync(numeric_only_failure):
        return _stdlib_socket.getaddrinfo(host, port, family, type, proto, flags | _NUMERIC_ONLY)
...
async def getnameinfo(sockaddr, flags):
    ...
    if hr is not None:
        ...
        return await trio.to_thread.run_sync(_stdlib_socket.getnameinfo, sockaddr, flags, cancellable=True)
...
async def getprotobyname(name):
    ...
    """
    return await trio.to_thread.run_sync(_stdlib_socket.getprotobyname, name, cancellable=True)
...
if sys.platform == "win32" or (not TYPE_CHECKING and hasattr(_stdlib_socket, "fromshare")):
...
def _make_simple_sock_method_wrapper(methname, wait_fn, maybe_avail=False):
    ...
    wrapper.__doc__ = f"""Like :meth:`socket.socket.{methname}`, but async.
            ...
            """
    if maybe_avail:
        wrapper.__doc__ += f"Only available on platforms where :meth:`socket.socket.{methname}` is " "available."
    return wrapper
...
class SocketType:
    def __init__(self):
        raise TypeError(
            "SocketType is an abstract class; use trio.socket.socket if you " "want to construct a socket object"
        )
...
class _SocketType(SocketType):
    def __init__(self, sock):
        if type(sock) is not _stdlib_socket.socket:
            ...
            raise TypeError("expected object of type 'socket.socket', not '{}".format(type(sock).__name__))
        self._sock = sock
    ...
    async def _resolve_address_nocp(self, address, flags):
        ...
        if self._sock.family == _stdlib_socket.AF_INET:
            ...
            if not isinstance(address, tuple) or not 2 <= len(address) <= 4:
                raise ValueError("address should be a (host, port, [flowinfo, [scopeid]]) tuple")
        ...
        gai_res = await getaddrinfo(host, port, self._sock.family, self.type, self._sock.proto, flags)
    ...
    recvfrom_into = _make_simple_sock_method_wrapper("recvfrom_into", _core.wait_readable)
    ...
    if hasattr(_stdlib_socket.socket, "recvmsg"):
        recvmsg = _make_simple_sock_method_wrapper("recvmsg", _core.wait_readable, maybe_avail=True)
    ...
    if hasattr(_stdlib_socket.socket, "recvmsg_into"):
        recvmsg_into = _make_simple_sock_method_wrapper("recvmsg_into", _core.wait_readable, maybe_avail=True)
    ...
    async def sendto(self, *args):
        ...
        return await self._nonblocking_helper(_stdlib_socket.socket.sendto, args, {}, _core.wait_writable)
    ...
    if sys.platform != "win32" or (not TYPE_CHECKING and hasattr(_stdlib_socket.socket, "sendmsg")):
        ...
        async def sendmsg(self, *args):
            ...
            if len(args) == 4 and args[-1] is not None:
                args = list(args)
                args[-1] = await self._resolve_remote_address_nocp(args[-1])
            return await self._nonblocking_helper(_stdlib_socket.socket.sendmsg, args, {}, _core.wait_writable)
