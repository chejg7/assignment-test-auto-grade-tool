...
async def ssl_echo_server_raw(**kwargs):
    ...
    async with trio.open_nursery() as nursery:
        ...
        with a, b:
            nursery.start_soon(trio.to_thread.run_sync, partial(ssl_echo_serve_sync, b, **kwargs))
...
async def test_ssl_server_basics(client_ctx):
    ...
    with a, b:
        ...
        server_transport = SSLStream(SocketStream(server_sock), SERVER_CTX, server_side=True)
        assert server_transport.server_side

        def client():
            with client_ctx.wrap_socket(a, server_hostname="trio-test-1.example.org") as client_sock:
...
async def test_renegotiation_randomized(mock_clock, client_ctx):
    ...
    with virtual_ssl_echo_server(client_ctx, sleeper=sleeper_with_slow_wait_writable_and_expect) as s:
        await send(b"x")
        s.transport_stream.renegotiate()
        async with _core.open_nursery() as nursery:
            nursery.start_soon(expect, b"x")
...
async def test_ssl_over_ssl(client_ctx):
    ...
    client_1 = SSLStream(client_0, client_ctx, server_hostname="trio-test-1.example.org")
    server_1 = SSLStream(server_0, SERVER_CTX, server_side=True)

    client_2 = SSLStream(client_1, client_ctx, server_hostname="trio-test-1.example.org")
...
async def test_SSLListener(client_ctx):
    async def setup(**kwargs):
        ...
        ssl_client = SSLStream(transport_client, client_ctx, server_hostname="trio-test-1.example.org")
