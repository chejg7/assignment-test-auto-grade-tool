...
async def test_basic(background_process):
    ...
    assert repr(proc) == "<trio.Process {!r}: {}>".format(EXIT_FALSE, "exited with status 1")
...
async def test_async_with_basics_deprecated(recwarn):
    async with await open_process(CAT, stdin=subprocess.PIPE, stdout=subprocess.PIPE) as proc:
        pass
...
COPY_STDIN_TO_STDOUT_AND_BACKWARD_TO_STDERR = python(
    "data = sys.stdin.buffer.read(); " "sys.stdout.buffer.write(data); " "sys.stderr.buffer.write(data[::-1])"
)
            ...
            async with _core.open_nursery() as nursery:
                ...
                async def drain_one(stream, count, digit):
                    while count > 0:
                        ...
                        assert result == ("{}".format(digit).encode("utf-8") * len(result))
                        count -= len(result)
                    assert count == 0
...
async def test_run():
    ...
    result = await run_process(CAT, stdin=data, capture_stdout=True, capture_stderr=True)
...
async def test_run_check():
    ...
    result = await run_process(cmd, capture_stdout=True, capture_stderr=True, check=False)
...
async def test_run_with_broken_pipe():
    result = await run_process([sys.executable, "-c", "import sys; sys.stdin.close()"], stdin=b"x" * 131072)
    assert result.returncode == 0
...
async def test_stderr_stdout(background_process):
    ...
    # is not redirected
    async with background_process(CAT, stdin=subprocess.PIPE, stderr=subprocess.STDOUT) as proc:
