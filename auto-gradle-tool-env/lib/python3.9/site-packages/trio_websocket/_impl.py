...
async def connect_websocket(
    ...
    *,
    ...
    logger.debug("Connecting to ws%s://%s:%d%s", "" if ssl_context is None else "s", host, port, resource)
    if ssl_context is None:
        stream = await trio.open_tcp_stream(host, port)
    else:
        stream = await trio.open_ssl_over_tcp_stream(host, port, ssl_context=ssl_context, https_compatible=True)
    if port in (80, 443):
        host_header = host
    else:
    ...
    connection = WebSocketConnection(
        ...
        client_extra_headers=extra_headers,
        message_queue_size=message_queue_size,
        max_message_size=max_message_size,
    )
...
async def wrap_server_stream(
    nursery, stream, message_queue_size=MESSAGE_QUEUE_SIZE, max_message_size=MAX_MESSAGE_SIZE
):
    """
    ...
    connection = WebSocketConnection(
        stream, wsproto, message_queue_size=message_queue_size, max_message_size=max_message_size
    )
    nursery.start_soon(connection._reader_task)
    ...
    *,
    ...
    if ssl_context is None:
        ...
        open_tcp_listeners = partial(
            trio.open_ssl_over_tcp_listeners, port, ssl_context, host=host, https_compatible=True
        )
...
class ConnectionRejected(HandshakeError):
    ...
    def __repr__(self):
        """Return representation."""
        return "{}<status_code={}>".format(self.__class__.__name__, self.status_code)
...
class CloseReason:
    ...
    def __repr__(self):
        ...
        return "{}<code={}, name={}, reason={}>".format(self.__class__.__name__, self.code, self.name, self.reason)
...
class WebSocketConnection(trio.abc.AsyncResource):
        ...
        *,
        ...
        if wsproto.client:
            self._initial_request = Request(
                host=host, target=path, subprotocols=client_subprotocols, extra_headers=client_extra_headers or []
            )
        else:
    ...
    async def _get_request(self):
        ...
        if self._connection_proposal is None:
            raise Exception("No proposal available. Did you call this method" " multiple times or at the wrong time?")
    ...
    async def _handle_message_event(self, event):
        ...
        if self._message_size > self._max_message_size:
            err = "Exceeded maximum message size: {} bytes".format(self._max_message_size)
            self._message_size = 0
            self._message_parts = []
...
class Endpoint:
    ...
    def url(self):
        ...
        if (self.port == 80 and not self.is_ssl) or (self.port == 443 and self.is_ssl):
            port_str = ""
        else:
    ...
    def __repr__(self):
        ...
        return 'Endpoint(address="{}", port={}, is_ssl={})'.format(self.address, self.port, self.is_ssl)
...
class WebSocketServer:
    ...
    def port(self):
        ...
        if len(self._listeners) > 1:
            raise RuntimeError("Cannot get port because this server has" " more than 1 listeners.")
        try:
            listener = self.listeners[0]
            return listener.port
        except AttributeError:
            raise RuntimeError("This socket does not have a port: {!r}".format(listener)) from None
    ...
    async def run(self, *, task_status=trio.TASK_STATUS_IGNORED):
        ...
        async with trio.open_nursery() as nursery:
            serve_listeners = partial(
                trio.serve_listeners, self._handle_connection, self._listeners, handler_nursery=self._handler_nursery
            )
            await nursery.start(serve_listeners)
            logger.debug("Listening on %s", ",".join([str(l) for l in self.listeners]))
    ...
    async def _handle_connection(self, stream):
        ...
        async with trio.open_nursery() as nursery:
            wsproto = WSConnection(ConnectionType.SERVER)
            connection = WebSocketConnection(
                stream, wsproto, message_queue_size=self._message_queue_size, max_message_size=self._max_message_size
            )
            nursery.start_soon(connection._reader_task)
