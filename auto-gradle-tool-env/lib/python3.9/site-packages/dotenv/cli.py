...
def list(ctx: click.Context, format: bool) -> None:
    ...
    if not os.path.isfile(file):
        raise click.BadParameter('Path "%s" does not exist.' % (file), ctx=ctx)
...
def run(ctx: click.Context, override: bool, commandline: List[str]) -> None:
    ...
    file = ctx.obj["FILE"]
    if not os.path.isfile(file):
        raise click.BadParameter("Invalid value for '-f' \"%s\" does not exist." % (file), ctx=ctx)
    dotenv_as_dict = {
        k: v for (k, v) in dotenv_values(file).items() if v is not None and (override or k not in os.environ)
    }
