...
class H11Handshake:
    ...
    def send(self, event: Event) -> bytes:
        ...
        if isinstance(event, Request):
            ...
            raise LocalProtocolError(f"Event {event} cannot be sent during the handshake")
        return data
    ...
    def _accept(self, event: AcceptConnection) -> bytes:
        ...
        response = h11.InformationalResponse(status_code=101, headers=headers + event.extra_headers)
    ...
    def _reject(self, event: RejectConnection) -> bytes:
        if self.state != ConnectionState.CONNECTING:
            raise LocalProtocolError("Connection cannot be rejected in state %s" % self.state)
    ...
    def _establish_client_connection(self, event: h11.InformationalResponse) -> AcceptConnection:  # noqa: MC0001
        ...
        self._state = ConnectionState.OPEN
        return AcceptConnection(extensions=extensions, extra_headers=headers, subprotocol=subprotocol)

    def __repr__(self) -> str:
        return "{}(client={}, state={})".format(self.__class__.__name__, self.client, self.state)
...
def client_extensions_handshake(accepted: Iterable[str], supported: Sequence[Extension]) -> List[Extension]:
    ...
    for accept in accepted:
        ...
        for extension in supported:
            ...
            raise RemoteProtocolError(f"unrecognized extension {name}", event_hint=RejectConnection())
    return extensions
