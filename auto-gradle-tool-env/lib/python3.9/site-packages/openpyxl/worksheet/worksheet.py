...
class Worksheet(_WorkbookChild):
    ...
    def __getitem__(self, key):
        ...
        if min_row is None:
            ...
            return cols
        if min_col is None:
            rows = tuple(self.iter_rows(min_col=min_col, min_row=min_row, max_col=self.max_column, max_row=max_row))
            if min_row == max_row:
                rows = rows[0]
            return rows
        if ":" not in key:
            return self._get_cell(min_row, min_col)
        return tuple(self.iter_rows(min_row=min_row, min_col=min_col, max_row=max_row, max_col=max_col))
    ...
    def merge_cells(self, range_string=None, start_row=None, start_column=None, end_row=None, end_column=None):
        ...
        if range_string is None:
            cr = CellRange(
                range_string=range_string, min_col=start_column, min_row=start_row, max_col=end_column, max_row=end_row
            )
    ...
    def _invalid_row(self, iterable):
        raise TypeError(
            "Value must be a list, tuple, range or generator, or a dict. Supplied value is {0}".format(type(iterable))
        )
