...
class WorkSheetParser(object):
    def __init__(
        self, src, shared_strings, data_only=False, epoch=WINDOWS_EPOCH, date_formats=set(), timedelta_formats=set()
    ):
        ...
        if not self.data_only and element.find(FORMULA_TAG) is not None:
            ...
            if data_type == "n":
                ...
                if style_id in self.date_formats:
                    data_type = "d"
                    try:
                        value = from_excel(value, self.epoch, timedelta=style_id in self.timedelta_formats)
