...
class NamedStyle(Serialisable):
    ...
    def _recalculate(self):
        ...
        if fmt in BUILTIN_FORMATS_REVERSE:
            ...
            fmt = self._wb._number_formats.add(self.number_format) + (BUILTIN_FORMATS_MAX_SIZE)
    ...
    def as_name(self):
        ...
        """
        named = _NamedCellStyle(name=self.name, builtinId=self.builtinId, hidden=self.hidden, xfId=self.xfId)
        return named
...
class _NamedCellStyleList(Serialisable):
    ...
    def names(self):
        ...
        for ns in sorted(self.cellStyle, key=sort_fn):
            ...
            style = NamedStyle(name=ns.name, hidden=ns.hidden, builtinId=ns.builtinId)
            names.add(ns.name)
