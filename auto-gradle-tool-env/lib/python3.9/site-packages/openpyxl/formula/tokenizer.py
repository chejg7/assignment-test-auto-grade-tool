...
class Tokenizer(object):
    ...
    def _parse_brackets(self):
        ...
        lefts = [(t.start(), 1) for t in re.finditer(r"\[", self.formula[self.offset :])]
        rights = [(t.start(), -1) for t in re.finditer(r"\]", self.formula[self.offset :])]
    ...
    def _parse_operator(self):
        ...
        if self.formula[self.offset : self.offset + 2] in (">=", "<=", "<>"):
            self.items.append(Token(self.formula[self.offset : self.offset + 2], Token.OP_IN))
            return 2
        ...
        # From here on, curr_char is guaranteed to be in '+-'
            ...
            prev = next((i for i in reversed(self.items) if i.type != Token.WSPACE), None)
            is_infix = prev and (
                prev.subtype == Token.CLOSE or prev.type == Token.OP_POST or prev.type == Token.OPERAND
            )
            if is_infix:
                token = Token(curr_char, Token.OP_IN)
            else:
