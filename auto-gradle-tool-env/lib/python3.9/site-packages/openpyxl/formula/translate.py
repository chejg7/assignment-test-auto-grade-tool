...
class Translator(object):
    ...
    def translate_range(cls, range_str, rdelta, cdelta):
        ...
        if match is not None:
            return (
                ws_part + cls.translate_row(match.group(1), rdelta) + ":" + cls.translate_row(match.group(2), rdelta)
            )
        match = cls.COL_RANGE_RE.match(range_str)  # e.g. `A:BC`
        if match is not None:
            return (
                ws_part + cls.translate_col(match.group(1), cdelta) + ":" + cls.translate_col(match.group(2), cdelta)
            )
