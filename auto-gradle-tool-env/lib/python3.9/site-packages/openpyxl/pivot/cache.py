...
from openpyxl.packaging.relationship import RelationshipList, Relationship, get_rels_path
...
class OLAPSet(Serialisable):
    ...
    sortType = NoneSet(
        values=(
            ["ascending", "descending", "ascendingAlpha", "descendingAlpha", "ascendingNatural", "descendingNatural"]
        )
    )
    queryFailed = Bool()
    tpls = Typed(expected_type=TupleList, allow_none=True)
